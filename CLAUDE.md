...
- Ciphers support Scandinavian alphabets (Æ/Ø/Å, Danish/Norwegian/Swedish variants) via a per-cipher alphabet dropdown reading `CAESAR_ALPHABETS` — Caesar, A1Z26, Affine, Playfair, Four-Square, Bifid, Nihilist, Polybius, Bacon, Morse, Beaufort, Autokey, Gronsfeld, Keyword Substitution, ADFGVX, and the brute-force helper all follow this pattern. Gronsfeld is a Vigenère variant keyed by digits (0-9); Four-Square uses two keyword grids and Bifid a keyword-seeded Polybius square (both reuse the Playfair/Polybius grid geometry). ADFGVX combines a keyword-seeded 6×6 grid (`adfgvxFill(variant)` fits 26 letters + digits 0–9 for English, or the 29 Scandinavian letters + digits 0–6) with a keyed columnar transposition that reuses `columnOrder`/`columnHeight`. Nihilist adds a repeating key-phrase's numbers on top of keyword-Polybius coordinates (two keywords: one seeds the square via `bifidSquare`, one is the additive key). A few ciphers are English-only by mathematical necessity and so have no variant dropdown: Porta (reciprocal, needs an even alphabet for the half-split), Trifid (a 3×3×3 cube needs exactly 27 symbols = 26 letters + a full stop), and Hill (2×2 matrix mod 26; `modInverse` gates decodability on the determinant being coprime with 26, flagged inline by `syncHillMatrix` in `app.js`). The transposition ciphers (Columnar, Scytale) are position-based and alphabet-agnostic, so Scandinavian characters pass through untouched. Younger Futhark is a lossy 16-rune transliteration that maps Æ/Ø/Å (and Ä/Ö) onto their nearest runes. Some (Playfair/Polybius) switch grid dimensions for the 29-letter alphabets; Affine's modulus becomes 29 (prime, so every multiplier is valid — the `a` dropdown is rebuilt from `affineCoprimes(m)`). Retired registry ids (e.g. `scandicaesar`, merged into `caesar`) are mapped forward in `migrateCipherId()` in `state.js`.
- `dist/`, `build/`, `AegisRoot.cer`, and `*.exe` are build outputs and are not committed (`BasementenAegis.spec` is the one committed spec file). Built exes are distributed by attaching them to GitHub Releases; the README download link tracks the latest release automatically. When cutting a release, add the notes to `CHANGELOG.md` under a `## vX.Y.Z` heading and bump `APP_VERSION` in `version.js` to match the tag (it's displayed in the app footer), then push the tag. `.github/workflows/release.yml` verifies the tag matches `version.js` and drafts the GitHub Release from the matching `CHANGELOG.md` section; the exe must be signed locally and attached to the draft before publishing (so the draft stays private until the download link can resolve).
